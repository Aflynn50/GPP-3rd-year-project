...
public final class GameServer extends Thread implements Subject
{
  ...
  private synchronized List<List<Move>> getLegalMoves() throws MoveDefinitionException
  {
	List<List<Move>> legalMovesAllPlayers = new ArrayList<>();
	for (int i = 0; i < hosts.size(); i++)
	{
	  List<Move> legalMoves = stateMachine.getLegalMoves(currentState,stateMachine.getRoles()[i]);
	  legalMovesAllPlayers.add(legalMoves);
	}
	return legalMovesAllPlayers;
  }
